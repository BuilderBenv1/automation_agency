...
**2. sameAs** — the founder `Person` node now carries `sameAs: ["https://www.linkedin.com/in/benjamin-horne-8413b03a9/"]`. The `ProfessionalService` `sameAs` is **still `[]`** — see Deferred.
...
| Item | Reason |
|---|---|
| **`ProfessionalService.sameAs` is still `[]`** — the single highest-leverage AEO field | Both bracket placeholders in the brief were left unfilled: `[PASTE GBP URL]` and `[PASTE LINKEDIN COMPANY PAGE URL]`. The founder's personal LinkedIn was supplied and is on the `Person` node, where it belongs — putting a personal profile on the organisation node would be wrong. The TODO in `app/layout.tsx` now names exactly the two URLs required. **Unblocking this needs the GBP claimed (audit fix #28) and a LinkedIn company page URL.** |
